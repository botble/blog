---
title: "Buy Botble Products Direct and Save 30% (vs CodeCanyon)"
description: "Buy our Laravel themes and plugins on marketplace.botble.com and pay 30% less than the marketplace price. Same product, same license, same support. Here is how it works and what it costs."
categories:
  - Deals
  - Buyer Guides
tags:
  - botble
  - direct-purchase
  - discount
  - laravel
  - ecommerce
  - codecanyon
  - license
image: https://botble.com/storage/news/buy-botble-direct-save-30-percent-hero.jpg
status: published
is_featured: true
---

![Buy Botble products direct and save 30%](https://botble.com/storage/news/buy-botble-direct-save-30-percent-hero.jpg)

# Buy Botble Products Direct and Save 30%

I'm Sang from Botble. Quick point I wanted to make: if you buy our themes and plugins straight from us at [marketplace.botble.com/portfolio](https://marketplace.botble.com/portfolio), you pay 30% less than the marketplace price. Same files, same license, same support.

There's no coupon to enter either. The lower price is already on every product page.

## Why it's cheaper

Marketplaces take a large cut of every sale. Buy from us directly and that cut disappears, so we hand most of it back to you as a lower price. You spend less, and we keep a little more to keep improving the product.

Everything else is exactly what you'd get from a marketplace purchase:

- The product itself: identical files, same version, updated at the same time.
- A real license key, issued on license.botble.com, valid for updates and support.
- Six months of support, plus lifetime updates.
- Free installation if you want us to set it up.

## What it costs

Here are some of the more popular products at the direct price:

| Product | Type | Marketplace | Direct (-30%) |
|---|---|---:|---:|
| [MartFury](https://marketplace.botble.com/portfolio/martfury) | Multi-vendor marketplace | ~~$349~~ | **$244.30** |
| [Shofy](https://marketplace.botble.com/portfolio/shofy) | eCommerce theme | ~~$59~~ | **$41.30** |
| [Flex Home](https://marketplace.botble.com/portfolio/flex-home) | Furniture and decor theme | ~~$59~~ | **$41.30** |
| [Nest](https://marketplace.botble.com/portfolio/nest) | Grocery and organic theme | ~~$59~~ | **$41.30** |
| [Homzen](https://marketplace.botble.com/portfolio/homzen) | Real estate theme | ~~$49~~ | **$34.30** |
| [Carento](https://marketplace.botble.com/portfolio/carento) | Car rental theme | ~~$59~~ | **$41.30** |
| [Zelio](https://marketplace.botble.com/portfolio/zelio) | Laravel theme | ~~$39~~ | **$27.30** |
| [Wholesale (B2B)](https://marketplace.botble.com/portfolio/wholesale) | B2B plugin | ~~$39~~ | **$27.30** |
| [Loyalty Points](https://marketplace.botble.com/portfolio/loyalty-points) | Rewards plugin | ~~$39~~ | **$27.30** |
| [Product Gifts](https://marketplace.botble.com/portfolio/product-gifts) | Gifts and promo plugin | ~~$29~~ | **$20.30** |

That's just a sample. There are more than 30 themes and plugins on the storefront, and the 30% covers all of them. You can see the full list at [marketplace.botble.com/portfolio](https://marketplace.botble.com/portfolio).

## How to buy

1. Open the storefront at [marketplace.botble.com/portfolio](https://marketplace.botble.com/portfolio).
2. Open a product and try its live demo. Every product has one.
3. Click Buy and leave your name and email. You don't need to create an account.
4. We reply with the payment details and the final price.

## How to pay

Use whatever is easiest for you:

- PayPal, which most buyers outside Vietnam find simplest.
- Wise, if you prefer a bank transfer with good rates.
- Local bank transfer or VietQR if you're in Vietnam.

We send the exact details once you submit the form. Pay the amount, then reply so we know.

## After you pay

As soon as the money arrives, you get your license key from license.botble.com, the download link for the latest version, and a short install guide in English or Vietnamese. If you'd rather not do the install yourself, we'll do it for you. Most orders go out the same day.

## Is it safe to buy this way?

Yes. You're buying from the people who actually build the product. The storefront is our own site, the license is genuine, and you always get the newest files. If anything is unclear, reply to the delivery email and it comes straight to me.

## Go take a look

Head over to [marketplace.botble.com/portfolio](https://marketplace.botble.com/portfolio), find what you need, and click Buy. You'll pay 30% less than you would anywhere else, and you'll be supporting the small team that makes and maintains the product.

Have a question before you buy? Just send it over. I read every message.

Thanks,
Sang, Botble Technologies
